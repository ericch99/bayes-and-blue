...
def get_G(input_shape):
    ...
    for i in range(16):
        ...
        nn = BatchNorm2d(act=lrelu, gamma_init=g_init)(nn)
    ...
    n = SubpixelConv2d(scale=2, n_out_channels=None, act=lrelu)(n)
    ...
    nn = Conv2d(input_shape[3], (1, 1), (1, 1), act=tf.nn.tanh, padding='SAME', W_init=w_init)(n)
...
def SRGAN_d2(input_shape):
    ...
    nin = Input(input_shape)
    n = Conv2d(64, (3, 3), (1, 1), act=lrelu, padding='SAME', W_init=w_init, name='n64s1/c')(nin)

    n = Conv2d(64, (3, 3), (2, 2), act=lrelu, padding='SAME', W_init=w_init, b_init=b_init, name='n64s2/c')(n)
    n = BatchNorm2d(gamma_init=gamma_init)(n)

    n = Conv2d(128, (3, 3), (1, 1), act=lrelu, padding='SAME', W_init=w_init, b_init=b_init, name='n128s1/c')(n)
    n = BatchNorm2d(gamma_init=gamma_init)(n)

    n = Conv2d(128, (3, 3), (2, 2), act=lrelu, padding='SAME', W_init=w_init, b_init=b_init, name='n128s2/c')(n)
    n = BatchNorm2d(gamma_init=gamma_init)(n)

    n = Conv2d(256, (3, 3), (1, 1), act=lrelu, padding='SAME', W_init=w_init, b_init=b_init, name='n256s1/c')(n)
    n = BatchNorm2d(gamma_init=gamma_init)(n)

    n = Conv2d(256, (3, 3), (2, 2), act=lrelu, padding='SAME', W_init=w_init, b_init=b_init, name='n256s2/c')(n)
    n = BatchNorm2d(gamma_init=gamma_init)(n)

    n = Conv2d(512, (3, 3), (1, 1), act=lrelu, padding='SAME', W_init=w_init, b_init=b_init, name='n512s1/c')(n)
    n = BatchNorm2d(gamma_init=gamma_init)(n)

    n = Conv2d(512, (3, 3), (2, 2), act=lrelu, padding='SAME', W_init=w_init, b_init=b_init, name='n512s2/c')(n)
    n = BatchNorm2d(gamma_init=gamma_init)(n)

    n = Flatten()(n)
    n = Dense(n_units=1024, act=lrelu, name='d1024', W_init=w_init)(n)
    no = Dense(n_units=1, act = tf.nn.sigmoid, name='out', W_init=w_init)(n)
    D = Model(inputs= nin, outputs = no)

    return D


#conditional discriminator (takes in lr-hr pairs as input)
def get_D_Conditional(input_shape):
    ...
    nin_lr = Input((input_shape[0], input_shape[1]/8, input_shape[2]/8, input_shape[3]))
    nin_hr = Input(input_shape)

    n_hr = Conv2d(64, (3, 3), (2, 2), act=lrelu, padding='SAME', W_init=w_init)(nin_hr)
    n_hr = BatchNorm2d(gamma_init=gamma_init)(n_hr)
    n_hr = Conv2d(64, (3, 3), (2, 2), act=lrelu, padding='SAME', W_init=w_init)(n_hr)
    n_hr = BatchNorm2d(gamma_init=gamma_init)(n_hr)
    n_hr = Conv2d(64, (3, 3), (2, 2), act=lrelu, padding='SAME', W_init=w_init)(n_hr)
    n_hr = BatchNorm2d(gamma_init=gamma_init)(n_hr)
    

    n_lr = Conv2d(64, (3, 3), (1, 1), act=lrelu, padding='SAME', W_init=w_init)(nin_lr)
    n_lr = BatchNorm2d(gamma_init=gamma_init)(n_lr)
    n_lr = Conv2d(64, (3, 3), (1, 1), act=lrelu, padding='SAME', W_init=w_init)(n_lr)
    n_lr = BatchNorm2d(gamma_init=gamma_init)(n_lr)
    n_lr = Conv2d(64, (3, 3), (1, 1), act=lrelu, padding='SAME', W_init=w_init)(n_lr)
    n_lr = BatchNorm2d(gamma_init=gamma_init)(n_lr)
